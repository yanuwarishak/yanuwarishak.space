...
export function getAllProjects(directory) {
  ...
  projects.sort((a, b) => Number(new Date(b.publishedAt)) - Number(new Date(a.publishedAt)))
...
export function getIndividualPost(directory, slug) {
  const markdownWithMeta = fs.readFileSync(path.join(directory, slug + '.mdx'), 'utf-8')
