        ...
        <>
          {song.isPlaying ? (
            <div className="cursor-default justify-center items-center flex flex-col p-2">
              ...
              <p className="font-semibold text-sm md:text-normal mb-2">Now Playing:</p>
              <div className="relative mb-2">
                {song.image ? (
                  ...
                  <p className="w-20 h-20 bg-gray-400 text-center p-4 text-gray-700 text-4xl">X</p>
                )}
              </div>
              <div className="flex flex-col">
                {song.link && (
                  <a href={song.link} target="_blank" rel="noopener noreferrer">
                    <p className="font-semibold hover:underline cursor-pointer">{song.title}</p>
                  </a>
                )}
                {!song.link && <p className="font-semibold">{song.title}</p>}
                <p className="font-normal">by {song.artist ? song.artist : 'Local File'}</p>
              </div>
            </div>
          ) : (
          ...
          <div className="w-full flex flex-col items-center">
            ...
            <div className="cursor-pointer my-2" onClick={() => setOpen(!isOpen)}>
              <ExpandIcon />
            </div>
          </div>
        </>
